...
const TrackingPage = () => {
    ...
    const fetchTracking = async () => {
        try {
            // Check if tracking data is in localStorage
            const storedTracking = JSON.parse(localStorage.getItem(`trackingData_${id}`));
            if (storedTracking) {
            ...
            const data = await response.json();
            setTracking(data);
            localStorage.setItem(`trackingData_${id}`, JSON.stringify(data)); // Save data in localStorage
            setLoading(false);
        } catch (error) {
            console.error('Error fetching tracking data:', error);
            setLoading(false);
